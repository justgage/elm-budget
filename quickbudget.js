...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var update = F2(function (action,model) {    var _p0 = action;if (_p0.ctor === "Categorize") {    return model;} else {    return model;}});
   var Expense = F3(function (a,b,c) {    return {name: a,amount: b,cat: c};});
   var Budget = F2(function (a,b) {    return {old: a,$new: b};});
   var NoOp = {ctor: "NoOp"};
   var Categorize = {ctor: "Categorize"};
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,_U.list([]),_U.list([$Html.text($Basics.toString(model))]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Categorize)]),_U.list([$Html.text("cat1")]))]));
   });
   var model = {old: _U.list([]),$new: _U.list([])};
   var main = $StartApp$Simple.start({model: model,view: view,update: update});
   return _elm.Main.values = {_op: _op,main: main,model: model,view: view,Categorize: Categorize,NoOp: NoOp,Budget: Budget,Expense: Expense,update: update};
};
