...
Elm.QuickBudget.make = function (_elm) {
   ...
   var update = F2(function (action,model) {
      ...
      if (_p2.ctor === "[]") {
            ...
            return {$new: _p2._1,old: A2($List._op["::"],_p2._0,old)};
         }
   });
   var Categorize = function (a) {    return {ctor: "Categorize",_0: a};};
   var viewExpense = function (x) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("card")]),
      _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("")]),_U.list([$Html.text(x.name)]))
   ...
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,_U.list([]),A2($List.map,viewExpense,model.$new))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Categorize("cat"))]),_U.list([$Html.text("Home")]))]));
   });
   var model = {old: _U.list([]),$new: _U.list([{name: "Walmart",amount: 12.0,cat: "none"},{name: "Winco",amount: 18.0,cat: "none"}])};
